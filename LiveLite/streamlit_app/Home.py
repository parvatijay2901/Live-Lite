...
with col1:
   ...
   st.markdown("""<h4 style='text-align:center;'>Take some time to explore the features and resources available, 
               and embark on your journey towards improved health and wellness with us.</h4>""", 
               unsafe_allow_html=True)
   LiveLite.add_blank_lines()
   col11, col12 = st.columns([1,1])
   with col11:
      st.markdown("""<p style='text-align:center; color:#00b3b3; font-weight:bold;'>Delve into the multifaceted landscape
                  of obesity, exploring its complexities, causes, and consequences</p>""", 
                  unsafe_allow_html=True)
      
      button_1 = st.button('Understand Obesity 🤷🏻‍♀️', use_container_width=True, key='button4')
      if button_1:
         st.switch_page("pages/1_understanding_obesity.py")
   with col12:
      st.markdown("""<p style='text-align:center; color:#00b3b3; font-weight:bold;'>Receive valuable insights to
                  help you make informed decisions about your health and wellness journey</p>""",
                  unsafe_allow_html=True)
...
if 'food_nutrition_data' not in st.session_state or 'data_NHANES' not in st.session_state or 'data_IHME' not in st.session_state:
