...
def display_prevention_and_management():
    ...
    st.markdown(
        """
        Treatment of obesity can generally be divided into two stages. The initial treatment and subsequent treatments.
    ...
    st.markdown(
        ...
        An example of dietary weight-loss goals would be reducing energy intake by a certain amount of kcal/day 
        ...
        gain (Piercy et al., 2018).
        
        Behavior goals are equally important. These goals should within the indivuals control and also be SMART
        (Specific, Measurable, Achievable, Reasonable, and Time-bound). A simple example of a behavior goal would be
        to eat junk / fast food less than once per week.
    ...
    st.markdown(
        """
        Liposuction is another option available for individuals seeking to lose weight. However, this technique has
        mixed reviews from medical professionals.
        
        This is due to studies that have shown that weight loss by liposuction does not seem to bring the benefits that
        usually come with weight loss. For example, a study that examined the effects of liposuction found that
        although subject waist circumference improved, no improvements in risk factors of coronary heart disease such as
        high blood pressure or glucose presence in plasma were observed (Klein et al., 2004).
        """
    )
